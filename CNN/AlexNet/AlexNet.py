...
#    灰度扩 3 通道；使用 FMNIST 经验 mean/std 做标准化
...
# 3) 定义网络（更接近论文的 AlexNet）
class SimpleNN(nn.Module):
    def __init__(self):
        super(SimpleNN, self).__init__()
        self.net = nn.Sequential(
            # conv1: 96×11×11, stride=4, padding=0 → 55×55
            nn.Conv2d(3, 96, kernel_size=11, stride=4, padding=0), nn.ReLU(),
            nn.LocalResponseNorm(size=5, alpha=1e-4, beta=0.75, k=2.0),
            nn.MaxPool2d(kernel_size=3, stride=2),  # 55→27

            # conv2: 256×5×5, padding=2 → 27×27
            nn.Conv2d(96, 256, kernel_size=5, padding=2), nn.ReLU(),
            nn.LocalResponseNorm(size=5, alpha=1e-4, beta=0.75, k=2.0),
            nn.MaxPool2d(kernel_size=3, stride=2),  # 27→13

            # conv3/4/5（3×3, padding=1）
